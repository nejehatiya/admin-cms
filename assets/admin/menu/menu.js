...
let ajaxurl = "/admin4552255/api/menu-builder/";
(function($) {
    let menus = {
	...
	api = window.wpNavMenu = {
		...
		initSortables : function() {
			...
			api.menuList.sortable({
				...
				change: function(e, ui) {
					...
					updateSharedVars(ui);
                    console.log('ui 752',api.menuList);
				},
				sort: function(e, ui) {
		...
		addItemToMenu : function(menuItem, processMethod, callback) {
			...
			callback = callback || function(){};
            let object_keys = Object.keys(menuItem);
            console.log('menuItem',);
			params = {
				'action': 'add-menu-item',
				'menu': menu,
				'menu-settings-column-nonce': nonce,
				'menu-item': JSON.stringify([menuItem[object_keys[object_keys.length-1]]])
			};

			$.post( ajaxurl, params, function(menuMarkup) {
                console.log('menuMarkup',menuMarkup);
                $("#menu-to-edit").append(menuMarkup.menu_item);
                $("#menu-to-edit").sortable('refresh');
				$(".add-to-menu .spinner ").removeClass('is-active')
		...
		eventOnClickMenuSave : function(e) {
			...
			window.onbeforeunload = null;
            console.log('testeee');
            main_function.structureMenu();
			return false;
		},

		eventOnClickMenuDelete : function() {
			// Delete warning AYS.
			if ( window.confirm( 'You are about to permanently delete this menu.\n\'Cancel\' to stop, \'OK\' to delete.' )  ) {
				window.onbeforeunload = null;
				return true;
			}
			return false;
		...
		eventOnClickMenuItemDelete : function(clickedEl) {
			...
			/*$(document).on('click',".submitbox .submitdelete ",function(e){
				alert('oki');
				e.preventDefault();
