...
class ApiConfigurationController extends AbstractController
{
    ...
    #[Route('/', name: 'app_configuration_api_edit_config', methods: ['POST'])]
    public function indexEdit(Request $request): Response
    {
        $this->em->getRepository(Configuration::class)->deleteAllRecords();
        //sleep(1);
        $this->em->getRepository(Configuration::class)->resetAutoIncrement();
        // 1 - create config json
        $configuration = new Configuration();
        $configuration->setConfigName('configuration_json');
        $configuration->setConfigValue($request->request->get('config_data'));
            ...
            foreach($config_data_list as $key=>$value){
                if(is_array($value)){
                    foreach($value as $k=>$v){
                        $new_meta = new Configuration();
                        $new_meta->setConfigName($k);
                        $new_meta->setConfigValue($v);
        ...
        // publish in database
        $this->em->flush();
        return $this->json(['success'=>true,'message'=>'config mis à jour avec succés']);
    }
}
